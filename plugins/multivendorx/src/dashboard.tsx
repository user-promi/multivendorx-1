...
const getEndpoints = (): TabDefinition[] => {
  const context = require.context('./dashboard', true, /index\.(ts|tsx|js|jsx)$/);
...
const RouteRenderer: React.FC<{ currentTab: string; endpoints: TabDefinition[] }> = ({ currentTab, endpoints }) => {
  ...
  return (
    <div className="vendor-dashboard">
      <ul className="dashboard-tabs">
        {endpoints.map((e) => (
          <li key={e.tab} className={currentTab === e.tab ? 'current' : ''}>
            <a
              href={`?tab=${e.tab}`}
              onClick={(eClick) => {
      ...
      <div className="tab-content">
        <RouteRenderer currentTab={currentTab} endpoints={endpoints} />
      </div>
    </div>
  );
};

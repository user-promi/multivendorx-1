...
const BusinessAddress = () => {
	...
	const loadMapboxScript = () => {
		...
		mapboxGlScript.onerror = ( error ) => {
			setErrorMsg(
				'Failed to load Mapbox. Please check your internet connection.'
			);
		};
	};
	...
	const reverseGeocode = (
		...
		if ( provider === 'google' ) {
			...
			fetch(
				...
				.catch( () =>
					setErrorMsg( 'Reverse geocoding request failed' )
				);
		}
	};
	...
	const autoSave = ( updatedData: any ) => {
		...
		const saveData = {
			...updatedData,
			location_address:
				updatedData.location_address || updatedData.address || '',
			...
			<div className="form-group-wrapper">
				<div className="form-group">
					...
					<BasicInput
						...
						onChange={ ( e ) =>
							handleChange( 'location_address', e.target.value )
						}
					/>

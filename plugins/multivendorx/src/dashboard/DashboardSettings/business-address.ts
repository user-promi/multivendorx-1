...
const mapType =
	appLocalizer.settings_databases_value.geolocation.choose_map_api;
...
export default {
	...
	modal: [
		...
		//map component load
		{
			type: mapType,
			key: 'location',
			apiKey: appLocalizer.settings_databases_value.geolocation[
				`${mapType}_api_key`
			],
			label: __('Set Location on Map', 'multivendorx'),
			description: __(
				'Drag the marker to set your business location accurately.',
				'multivendorx'
			),
			required: true,
		},

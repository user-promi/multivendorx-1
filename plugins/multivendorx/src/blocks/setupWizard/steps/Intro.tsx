...
const sectionsData: Section[] = [
    ...
    },
    {
        ...
        steps: [
            { title: 'Pick a Color Scheme', description: '', completed: false, actionText: 'Set Up' },
            { title: 'Choose Typography', description: '', completed: false, actionText: 'Set Up' },
        ],
    },
];
    ...
    return (
        <div className="wizard-container">
            {sectionsData.map((section, idx) => {
                ...
                return (
                    <div key={idx} className={`wizard-section ${expanded === idx ? 'expanded' : ''}`}>
                        <div className="wizard-header" onClick={() => toggleSection(idx)}>
                            <div className="wizard-title">
                                <span className={`chevron ${expanded === idx ? 'rotate' : ''}`}>&gt;</span>
                                {section.title}
                            </div>
                                ...
                                {section.steps.map((task, tidx) => (
                                    <div key={tidx} className="wizard-step">
                                        <div className="step-info">
                                            <div className={`step-status ${task.completed ? 'completed' : ''}`}>
                                                {task.completed && <span className="check">&#10003;</span>}
                                            </div>
                                            <div className="step-text">
                                                <span className="step-title">{task.title}</span>
                                                {task.description && <span className="step-desc">{task.description}</span>}
                                            </div>
                                        </div>
                                        <button className="step-action">{task.actionText} &rarr;</button>
                                    </div>
                                ))}
                            </div>
                        )}

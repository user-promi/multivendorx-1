...
const AIField: React.FC<AIFieldProps> = ({ product, setProduct, field, openGeneratePopup, AIRefresh }) => {
    ...
    useEffect(() => {
        try {
            ...
            if (productData?.[storageKey]?.length) {
                ...
                setSuggestions(productData[storageKey].filter((s: string) => s?.trim()));
            } else {
                setSuggestions([]);
            }
        } catch (e) {
    ...
    useEffect(() => {
        if (suggestions.length > 0 && product?.[field]) {
            ...
            const index = suggestions.findIndex(suggestion => suggestion === currentValue);

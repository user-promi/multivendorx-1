jQuery(document).ready( function($) {
	...
	function initialize() {
  		if( !mvx_checkout_map_options.mapbox_emable ) {
			var latlng = new google.maps.LatLng( mvx_checkout_map_options.default_lat, mvx_checkout_map_options.default_lng );
			...
			center: [mvx_checkout_map_options.default_lng, mvx_checkout_map_options.default_lat],
			zoom: parseInt( mvx_checkout_map_options.default_zoom )
			});
			...
			map.on('load', function() {
				geocoder_mapbox.on('result', function(ev) {
					...
					document.getElementById("mvx_user_location_lat").value = ev.result.center[1];
					document.getElementById("mvx_user_location_lng").value = ev.result.center[0];					

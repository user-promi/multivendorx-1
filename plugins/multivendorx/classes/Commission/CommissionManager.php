...
class CommissionManager {
        ...
        if ( $order ) {
            ...
            if( $recalculate ) {
                foreach ( $order->get_items() as $item_id => $item ) {
                    ...
                    $commission_rate = [
                        'mode' => MultiVendorX()->setting->get_setting('revenue_sharing_mode'),
            ...
            $include_coupon     = 0 < $order->get_total_discount() && isset( MultiVendorX()->setting->get_setting('commission_include_coupon') );
    ...
    public function get_item_commission( $product_id, $item_id, $item, $order, $vendor ) {
        ...
        if ( MultiVendorX()->vendor_caps->vendor_payment_settings( 'commission_include_coupon' ) ) {
            $line_total = $order->get_item_total( $item, false, false ) * $item['qty'];
            if ( MultiVendorX()->vendor_caps->vendor_payment_settings( 'admin_coupon_excluded' ) && !$order_coupon_author_is_vendor ) {
        ...
        if ( $product_id && $vendor ) {
            ...
            if ( isset( MultiVendorX()->setting->get_setting( 'revenue_sharing_mode' ) ) ) {

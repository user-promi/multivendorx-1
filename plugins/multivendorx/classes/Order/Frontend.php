    ...
    public function woocommerce_my_account_my_orders_column_mvx_suborder( $order ) {
        ...
        if ($mvx_suborders) {
            ...
            foreach ($mvx_suborders as $suborder) {
                ...
                printf('<li><strong><a href="%s" title="%s">#%s</a></strong> &ndash; <small class="mvx-order-for-vendor">%s %s</small></li>', $order_uri, sanitize_title($suborder->get_status()), $suborder->get_order_number(), _x('for', 'Order table details', 'multivendorx'), $vendor->get('name')
                );

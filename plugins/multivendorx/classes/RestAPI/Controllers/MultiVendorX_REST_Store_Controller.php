        ...
        try {
            ...
            if ( $store_registration ) {
                ...
                foreach ( $rejected_stores as $store ) {
                    ...
                    $store_data [] = array(
                        'id'   => $store['ID'],
                        'note' => unserialize( $store_object->get_meta( Utill::STORE_SETTINGS_KEYS['store_reject_note'] ) ),
                    );
                }
            ...
            if ( $registrations ) {
                ...
                if ( MultiVendorX()->setting->get_setting( 'approve_store' ) == 'automatically' ) {
                    $current_user->set_role( 'store_owner' );
                        ...
                        $role = get_option( Utill::MULTIVENDORX_OTHER_SETTINGS['default_role'] );
    ...
    public function get_item( $request ) {
        ...
        try {
            ...
            $response = array(
                ...
                'create_time'        => date( 'M j, Y', strtotime( $store->get( Utill::STORE_SETTINGS_KEYS['create_time'] ) ) ),
                'commission'         => $commission,

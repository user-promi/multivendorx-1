...
class PaypalPayout {
    ...
    {
        $payment_settings = MultiVendorX()->setting->get_setting('payment_methods', []);
        $paypal_settings  = $payment_settings['paypal-payout'] ?? null;
        if ($paypal_settings && !empty($paypal_settings['enable'])) {
            $status = 'failed';
            $store = new Store($store_id);
            $receiver_email = $store->get_meta('payment_method') === 'paypal-payout'
                                ? $store->get_meta('paypal_email')
                                : "";
